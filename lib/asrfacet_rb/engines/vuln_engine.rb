...
module ASRFacet
  module Engines
    ...
    TAKEOVER_SIGNATURES = {
      ...
      def initialize(target, scan_results = {})
        @target = target
        @results = scan_results || {}
        @http_results = Array(@results[:http_responses] || @results[:http] || [])
        @cert_results = Array(@results[:certs] || @results[:ssl_certs] || [])
        @dns_results = Array(@results[:dns] || [])
        @subdomains = Array(@results[:subdomains] || [])
      ...
      def check_subdomain_takeover
        hosts = @dns_results.select { |record| (record[:type] || record["type"]).to_s.downcase == "cname" }.map do |record|
          [record[:host] || record["host"], record[:value] || record["value"]]
        end
        hosts.filter_map do |host, cname|
          next if host.to_s.empty? || cname.to_s.empty?
      ...
      def check_cors_misconfiguration
        @subdomains.filter_map do |subdomain|
          response = @client.get("https://#{subdomain}", headers: { "Origin" => "https://evil.com" })
          next if response.nil?

          acao = response["Access-Control-Allow-Origin"].to_s
          acac = response["Access-Control-Allow-Credentials"].to_s.downcase
          next unless acao == "https://evil.com" && acac == "true"

          cors_misconfiguration(subdomain)
        rescue StandardError
          nil
        end
      rescue StandardError
