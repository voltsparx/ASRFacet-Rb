...
function resolveTheme() {
  try {
    const persisted = window.localStorage.getItem("asrfacet-theme");
...
function applyTheme(theme) {
  ...
  try {
    window.localStorage.setItem("asrfacet-theme", state.theme);
  } catch (_) {}
}
...
function renderBuilderNotes(payload) {
  ...
  const notes = [
    ...
    {
      title: "Operator aids",
      copy: `${config.monitor ? "Monitoring is on" : "Monitoring is off"}, ${config.memory ? "recon memory is on" : "recon memory is off"}, ${config.headless ? "headless rendering is enabled" : "headless rendering is disabled"}, and ${config.adaptive_rate ? "adaptive rate control is active." : "adaptive rate control is disabled."}`
    }
  ];
