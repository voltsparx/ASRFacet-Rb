...
function renderBuilderNotes(payload) {
  ...
  const notes = [
    ...
    },
    {
      title: "Operator aids",
      copy: scannerMode ?
        `${config.scan_version ? "version detection is enabled" : "version detection is disabled"}, ${config.scan_os ? "OS detection is enabled" : "OS detection is disabled"}, and intensity is set to ${config.scan_intensity ?? 7}.` :
