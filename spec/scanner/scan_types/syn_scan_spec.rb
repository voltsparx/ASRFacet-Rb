...
RSpec.describe ASRFacet::Scanner::ScanTypes::SynScan do
  let(:probe_db) { instance_double(ASRFacet::Scanner::ProbeDB, service_for: "http") }
  let(:timing) { ASRFacet::Scanner::Timing.get(3) }

  it "maps SYN-ACK to open" do
    context = instance_double(ASRFacet::Scanner::ScanContext, timing: timing, probe_db: probe_db, tcp_prober: instance_double(ASRFacet::Scanner::Probes::TCPProber, send_probe: { reply: :syn_ack, window: 0 }))
  ...
  it "maps timeout to filtered" do
