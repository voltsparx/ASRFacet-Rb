...
RSpec.describe ASRFacet::Web::Server do
  it "renders the branded dashboard shell with external assets and workspace views" do
    ...
    expect(html).to include("Raw Backend")
    expect(html).to include("Version Intensity (0-9)")
    expect(html).to include("Plugins")
    expect(html).to include("Filters")
    expect(html).to include("Plugins Catalog")
    expect(html).to include("Filters Catalog")
    expect(html).to include("Engine and runtime status")
  ...
  it "exposes scanner and multi-format capabilities through bootstrap" do
    ...
    expect(payload.dig("capabilities", "raw_backends")).to include("auto", "nping", "builtin")
    expect(payload.dig("capabilities", "plugins")).to include("exposure_score", "attack_path")
    expect(payload.dig("capabilities", "filters")).to include("scope_guard", "duplicate_signal")
    expect(payload.dig("capabilities", "plugins_catalog")).to include(include("name" => "attack_path", "category" => "analysis"))
    expect(payload.dig("capabilities", "filters_catalog")).to include(include("name" => "scope_guard", "category" => "scope"))
    expect(payload.dig("capabilities", "selector_help")).to include("category:<name>", "mode:<mode>")
    expect(payload.dig("capabilities", "session_actions")).to include("clone", "stop", "delete")
